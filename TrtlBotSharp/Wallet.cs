...
namespace TrtlBotSharp
{
    public partial class TrtlBotSharp
    {
        ...
        public static async void BeginMonitoring()
        {
            ...
            while ((int)Result["knownBlockCount"] > ((int)Result["blockCount"]+10)) {
		Log(1, "Wallet", "Scanning for transactions and tip processing blocked. Scanned to {0:N0}/{1:N0}", (int)Result["blockCount"], (int)Result["knownBlockCount"]);
		walletReadyToTip = false;
                Result = Request.RPC(walletHost, walletPort, "getStatus", null, walletRpcPassword);
                await Task.Delay(walletUpdateDelay);
	    }
	    walletReadyToTip = true;
